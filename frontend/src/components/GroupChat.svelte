...
<script lang="ts">
  ...
  // let renderdChatData: TgGroupChat | null; // null = loading
  // let chatDataLoadingError: string | null = null;
...
<Group radius="sm">
  ...
  {:then loadChatResult}
    ...
    {:else}
      <Alert color="red" title="Ошибка загрузки данных чата">{loadChatResult.error}</Alert>
    {/if}
  {/await}
</Group>
