...
<div class="flex flex-col gap-5">
  <div class="flex flex-col gap-3">
    <h3 class="font-bold">Личные данные пользователь:ницы</h3>
    ...
    <div class="text-sm text-gray-600">
      ...
      {:else if config.user_attribution == "unique_id"}
        ...
        будет отличить ответы одно:й пользователь:ницы, но невозможно отследить е:ё личные данные.
      {:else if config.user_attribution == "name"}
        К ответу будет добавлено текущее имя телеграм-аккаунта пользователь:ницы
      {:else if config.user_attribution == "full"}
        К ответу будут добавлены полные данные телеграм аккаунта пользователь:ницы: имя, @юзернейм, user id
      {/if}
    </div>
  </div>
  <div class="flex flex-col gap-3">
