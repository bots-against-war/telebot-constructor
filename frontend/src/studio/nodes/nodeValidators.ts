...
export function validateHumanOperatorBlock(
    ...
    results.push(validateLocalizableText(config.feedback_handler_config.messages_to_user.forwarded_to_admin_ok, "ответ на успешно принятое сообщение", langConfig));
    results.push(validateLocalizableText(config.feedback_handler_config.messages_to_user.throttling, "предупреждение, что сообщений слишком много", langConfig));
