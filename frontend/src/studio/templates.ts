import type { CommandEntryPoint, NodeDisplayCoords, UserFlowConfig } from "../api/types";
...
export function contentOnlyTemplate(): Template {
  ...
  config.node_display_coords[contentBlockId] = { x: 100, y: 250 };
  return {
    config,
...
export function applyTemplate(config: UserFlowConfig, template: Template): UserFlowConfig {
  ...
  if (startCommand.next_block_id === null) {
    ...
    const existingBbox = boundingBox(config.node_display_coords);
    const templateBbox = boundingBox(template.config.node_display_coords);
    ...
    if (!(existingBbox.yMax < templateBbox.yMin || existingBbox.yMin > templateBbox.yMax)) {
      ...
      const xGap = 150;
      const xOffsetRight = Math.max(templateBbox.xMin - existingBbox.xMax + xGap, 0.0);
      const xOffsetLeft = Math.max(templateBbox.xMax - existingBbox.xMin + xGap, 0.0);
      xOffset = xOffsetRight < xOffsetLeft ? xOffsetRight : -xOffsetLeft;
    }
